...
- **Parse, don't validate.** If you're holding a value of the type, it is guaranteed correct. There
  is no "unchecked" variant floating around your codebase that might secretly be malformed.
- **Zero-cost.** Types are small, `Copy`, and allocation-free. Validating and formatting an
  identifier should not require a heap allocation.
...
This crate targets the Rust version pinned in `rust-toolchain.toml` at the repository root. Check
that file for the exact version this documentation was written against.